...
mod test {
    ...
    #[tokio::test]
    #[serial]
    async fn test_datanode_replacement() -> Result<()> {
        let _ = env_logger::builder().is_test(true).try_init();

        let _dfs = MiniDfs::with_features(&HashSet::from([DfsFeatures::HA]));
        
        // Create client with datanode replacement enabled (ALWAYS policy)
        let mut replace_dn_conf: HashMap<String, String> = HashMap::new();
        replace_dn_conf.insert(
            "dfs.client.block.write.replace-datanode-on-failure.enable".to_string(),
            "true".to_string(),
        );
        replace_dn_conf.insert(
        ...
        let client_always = Client::default_with_config(replace_dn_conf).unwrap();
        
        // Create client with datanode replacement disabled
        let mut disable_dn_conf: HashMap<String, String> = HashMap::new();
        disable_dn_conf.insert(
            "dfs.client.block.write.replace-datanode-on-failure.enable".to_string(),
            "false".to_string(),
        );
        let client_disabled = Client::default_with_config(disable_dn_conf).unwrap();
        
        // Create client with NEVER policy
        let mut never_dn_conf: HashMap<String, String> = HashMap::new();
        never_dn_conf.insert(
            "dfs.client.block.write.replace-datanode-on-failure.enable".to_string(),
            "true".to_string(),
        );
        never_dn_conf.insert(
        ...
        let client_never = Client::default_with_config(never_dn_conf).unwrap();
        
        // Test files
        let file_always = "/testfile_always";
        let file_disabled = "/testfile_disabled";
        let file_never = "/testfile_never";
        ...
        let mut writer = client_always
            .create(file_always, WriteOptions::default().replication(3))
            .await?;
            
        // Write first half of data
        writer.write(data.slice(..bytes_to_write / 2)).await?;
        
        // Give time for packets to send
        tokio::time::sleep(Duration::from_millis(100)).await;
        
        // Inject connection failure
        WRITE_CONNECTION_FAULT_INJECTOR.store(true, Ordering::SeqCst);
        
        // Write second half of data - this should trigger datanode replacement
        writer.write(data.slice(bytes_to_write / 2..)).await?;
        writer.close().await?;
        
        // Verify data was written correctly
        let reader = client_always.read(file_always).await?;
        check_file_content(&reader, data.clone()).await?;
        
        // Test with DISABLED policy
        let mut writer = client_disabled
            .create(file_disabled, WriteOptions::default().replication(3))
            .await?;
        ...
        // Write second half of data - this should still work but without replacement
        writer.write(data.slice(bytes_to_write / 2..)).await?;
        writer.close().await?;
        
        // Verify data was written correctly
        let reader = client_disabled.read(file_disabled).await?;
        check_file_content(&reader, data.clone()).await?;
        
        // Test with NEVER policy
        let mut writer = client_never
            .create(file_never, WriteOptions::default().replication(3))
            .await?;
        ...
        // Verify data was written correctly
        let reader = client_never.read(file_never).await?;
        check_file_content(&reader, data.clone()).await?;
