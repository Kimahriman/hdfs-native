...
impl From<PythonHdfsError> for PyErr {
    fn from(value: PythonHdfsError) -> Self {
        match value.0 {
            ...
            HdfsError::BlocksNotFound(path) => PyValueError::new_err(format!("Blocks not found for {}", path)),
            ...
            HdfsError::RPCError(class, msg) => PyRuntimeError::new_err(format!("RPC error: {}\n{}", class, msg)),
            HdfsError::FatalRPCError(class, msg) => PyRuntimeError::new_err(format!("Fatal RPC error: {}\n{}", class, msg)),
            HdfsError::SASLError(msg) => PyRuntimeError::new_err(format!("SASL error: {}", msg)),
            HdfsError::GSSAPIError(err) => PyRuntimeError::new_err(format!("GSSAPI error: {}", err.to_string())),
